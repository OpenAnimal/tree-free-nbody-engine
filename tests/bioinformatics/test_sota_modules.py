...
def test_elastic_spatial_hash_probe_bound():
    """Round-7 task T-A2 acceptance: probe_bound respected over 10^4 lookups."""
    print("\n" + "="*70)
    print("[TEST 0/19] ElasticSpatialHash3D probe-bound (funnel-hash façade)")
    print("="*70)
    from bioinformatics.core.elastic_spatial_hash import ElasticSpatialHash3D

    rng = np.random.RandomState(42)
    coords = rng.uniform(0.0, 60.0, size=(2000, 3))
    table = ElasticSpatialHash3D(cell_size=8.0, capacity_hint=4096, delta=0.05)
    morton_keys, unique_keys, inverse = table.build_from_coords(coords)
    assert table.count == len(unique_keys), (
        f"table.count {table.count} != unique keys {len(unique_keys)}"
    )
    bound = table.probe_bound
    assert bound > 0, "probe_bound must be positive"

    # 10^4 lookups (sampled from the inserted keys) must each respect the bound.
    # Uses the public `lookup_with_probes` API (T-B8 / R7-F29) rather than
    # poking the private `_table` attribute.
    n_lookups = 10_000
    idx = rng.randint(0, len(unique_keys), size=n_lookups)
    max_probes = 0
    for i in idx:
        val, probes = table.lookup_with_probes(int(unique_keys[i]))
        assert val is not None, "lookup of inserted key returned None"
        max_probes = max(max_probes, probes)
    assert max_probes <= bound, (
        f"max probes {max_probes} exceeded probe_bound {bound}"
    )
    print(f"  -> {n_lookups} lookups OK; max probes={max_probes} <= bound={bound}")
    print("[PASS] ElasticSpatialHash3D probe-bound verified.")


def test_born_radii_equivalence():
    """Round-7 task T-C3 acceptance: 2-ring gather matches all-atoms O(N^2)
    to <1e-12 on a 2,000-atom synthetic protein (identical sum, reorganized)."""
    print("\n" + "="*70)
    print("[TEST 0b/19] HCT Born radii: 2-ring gather vs O(N^2) equivalence")
    print("="*70)
    ...
    n_atoms = 2000
    coords = rng.uniform(0.0, 40.0, size=(n_atoms, 3))
    charges = rng.uniform(-0.5, 0.5, n_atoms)
    radii = rng.uniform(1.5, 2.0, n_atoms)
    masses = np.full(n_atoms, 12.0)
    atom_names = ["CA"] * n_atoms
    residue_names = ["GLY"] * n_atoms
    residue_ids = np.arange(1, n_atoms + 1, dtype=np.int32)
    chain_ids = ["A"] * n_atoms
    system = MolecularSystem(
        coords=coords, charges=charges, radii=radii, masses=masses,
        atom_names=atom_names, residue_names=residue_names,
        residue_ids=residue_ids, chain_ids=chain_ids,
    )

    # The gather ring must scale with cell_size: a hard ring=2 silently
    # misses descreening pairs in [2*cell_size, 16) Å for cell_size < 8.
    engine = SolvationFreeEnergyEngine(cell_size=8.0)
    born_radii = engine.compute_born_radii_hct(system)
    engine_small = SolvationFreeEnergyEngine(cell_size=4.0)
    born_radii_small = engine_small.compute_born_radii_hct(system)

    # Reference: brute-force O(N^2) HCT (the old implementation, inlined)
    vdw_radii = np.maximum(system.radii, 1.0)
    rho = vdw_radii - 0.09
    inv_born_ref = 1.0 / rho.copy()
    for i in range(n_atoms):
        ...
        inv_born_ref[i] -= np.sum(I_ij)
    inv_born_ref = np.maximum(inv_born_ref, 1.0 / 30.0)
    born_ref = 1.0 / inv_born_ref

    for label, got in (("cell_size=8", born_radii), ("cell_size=4", born_radii_small)):
        max_diff = float(np.max(np.abs(got - born_ref)))
        rel_diff = float(np.max(np.abs(got - born_ref) / np.maximum(np.abs(born_ref), 1e-30)))
        print(f"  -> N={n_atoms}, {label}: max abs diff={max_diff:.4e}, max rel diff={rel_diff:.4e}")
        assert max_diff < 1e-12, (
            f"Born radii mismatch ({label}): max abs diff {max_diff} >= 1e-12"
        )
    print("[PASS] Born radii ring gather matches O(N^2) to <1e-12 at cell_size 4 and 8.")


def test_yukawa_units():
    """Round-7 task T-C5: two-ion Yukawa units guard.

    Pins the Å↔unit-box mapping of `TaylorYukawaBioFMM` at machine precision
    with a two-ion analytic system. If the scaling factor `a`, `kappa_unit`,
    or `COULOMB_CONSTANT_KCAL / eps` drifts, this test fails.
    """
    print("\n" + "="*70)
    print("[TEST 0c/19] Two-ion Yukawa units guard (TaylorYukawaBioFMM)")
    print("="*70)
    from bioinformatics.core.fast_multipole_kernel import TaylorYukawaBioFMM

    # two ions, q = (+1, -1) e, r = 10 A, kappa = 0.127 1/A (150 mM), eps = 78.5
    r, kappa, eps = 10.0, 0.127, 78.5
    V_mag = 332.063711 / eps * np.exp(-kappa * r) / r   # |q2| * K * G(r)
    fmm = TaylorYukawaBioFMM(kappa, eps, cell_size_A=4.0, p=8)
    pot, _, _ = fmm.evaluate(
        np.array([[0.0, 0.0, 0.0], [r, 0.0, 0.0]]),
        np.array([1.0, -1.0]),
    )
    # ion 1 sees ion 2's charge: expected = q2 * K * exp(-kappa r) / (eps r)
    expected_0 = -V_mag   # sign follows q2 = -1
    expected_1 = +V_mag   # sign follows q1 = +1
    err_0 = abs(pot[0] - expected_0) / abs(expected_0)
    err_1 = abs(pot[1] - expected_1) / abs(expected_1)
    print(f"  -> pot[0]={pot[0]:.6e} vs expected={expected_0:.6e} (rel err {err_0:.2e})")
    print(f"  -> pot[1]={pot[1]:.6e} vs expected={expected_1:.6e} (rel err {err_1:.2e})")
    assert err_0 < 1e-8, f"ion 0 units drift: rel err {err_0} >= 1e-8"
    assert err_1 < 1e-8, f"ion 1 units drift: rel err {err_1} >= 1e-8"
    print("[PASS] Two-ion Yukawa units guard verified at <1e-8.")
...
def test_module_10_pangenome_search():
    ...
    # T-A3 / T-B8 assertion: the kmer table is backed by the funnel-hash
    # ElasticIntTable (not the legacy pre-funnel body).
    from core.elastic_hash import ElasticIntTable
    assert isinstance(pangenome.kmer_table._table, ElasticIntTable), (
        "KmerElasticHashTable must delegate to ElasticIntTable (T-A3)"
    )
...
def test_cross_validation_benchmarks():
    ...
    print("="*70)
    print("[SYNTHETIC SELF-GENERATED DATA — circular validation smoke test, not external accuracy]")
    ...
    print(f"  [SYNTHETIC SELF-GENERATED DATA] 5-Fold Pearson r: {skempi_report.mean_pearson_r:.3f} +/- {skempi_report.std_pearson_r:.3f}")
    ...
    print(f"  [SYNTHETIC SELF-GENERATED DATA] 5-Fold Mean ROC-AUC: {tap_report.mean_roc_auc:.3f}")
...
def test_kmer_vectorization_parity():
    """Parity check: vectorized ingest_sequence must produce identical k-mer
    counts to the legacy per-character Python loop with per-k-mer Python
    reverse-complement. Uses a seeded 20k-char random sequence (with sprinkled
    N's and mixed case) so the check is fast and deterministic."""
    print("\n" + "="*70)
    print("[TEST 0d/19] k-mer vectorized ingest parity (vs legacy per-char loop)")
    print("="*70)
    from bioinformatics.kmer_elastic_hash import KmerElasticHashTable, NUC_MAP, COMP_MAP

    def _legacy_ingest(table, sequence):
        """Verbatim copy of the pre-vectorization ingest_sequence body."""
        k = table.k
        k_mask = table.k_mask
        if len(sequence) < k:
            return 0
        ...
        for ch in sequence:
            if ch in NUC_MAP:
                val = NUC_MAP[ch]
                rolling_key = ((rolling_key << 2) | val) & k_mask
                valid_len += 1
                if valid_len >= k:
                    rev_key = 0
                    temp = rolling_key
                    for _ in range(k):
                        ...
                        temp >>= 2
                    canon = min(rolling_key, rev_key)
                    table.insert_or_increment(canon, increment=1)
                    total += 1
            else:
                rolling_key = 0
                valid_len = 0
        table.num_unique_kmers = table._table.count
        return total

    def _counts(table):
        return {int(kk): int(vv) for kk, vv in table._table.items()}

    rng = np.random.RandomState(2026)
    bases = np.array(["A", "C", "G", "T"])

    # Seeded 20k-char sequence with N's and mixed case for coverage.
    seq_arr = bases[rng.randint(0, 4, 20000)].astype(object)
    n_pos = rng.choice(20000, 200, replace=False)
    seq_arr[n_pos] = "N"
    seq = "".join(seq_arr)
    seq = seq[:10000].lower() + seq[10000:]  # mixed case

    for k in (11, 21):
        t_old = KmerElasticHashTable(k=k, capacity=max(1024, len(seq) * 2))
        n_old = _legacy_ingest(t_old, seq)
        c_old = _counts(t_old)

        t_new = KmerElasticHashTable(k=k, capacity=max(1024, len(seq) * 2))
        n_new = t_new.ingest_sequence(seq)
        c_new = _counts(t_new)

        assert n_old == n_new, f"k={k}: total_ingested {n_old} vs {n_new}"
        assert c_old == c_new, f"k={k}: count dict mismatch (sizes {len(c_old)} vs {len(c_new)})"
        print(f"  k={k}: {n_old} k-mers, {len(c_old)} unique — counts identical")

    print("[PASS] Vectorized k-mer ingest matches legacy per-char loop exactly.")


def test_toy_2cell_check_bio():
    """Wire in ``bioinformatics.core.fast_multipole_kernel.toy_2cell_check_bio``.

    The 2-cell toy check is a self-contained analytic regression for the
    bio-FMM Yukawa kernel: two opposite unit charges at a known separation,
    compared against the exact analytic potential.  It was defined but never
    called anywhere in the repo; this test invokes it and asserts its own
    ``rel-L2 < 1e-5`` acceptance gate.
    """
    from bioinformatics.core.fast_multipole_kernel import toy_2cell_check_bio
    ok = toy_2cell_check_bio()
    assert ok, "toy_2cell_check_bio failed its own rel-L2 < 1e-5 target"
    print("[PASS] toy_2cell_check_bio: rel-L2 < 1e-5 (2-cell Yukawa analytic check)")


if __name__ == "__main__":
    t_start = time.perf_counter()
    test_elastic_spatial_hash_probe_bound()
    test_born_radii_equivalence()
    test_yukawa_units()
    test_kmer_vectorization_parity()
    test_toy_2cell_check_bio()

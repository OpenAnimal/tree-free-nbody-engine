#!/usr/bin/env python3
"""Cross-validation of the multi-level uniform-grid adaptive FMM used by index.html.
...
def check_numerics(bits: int, z: np.ndarray) -> bool:
...
def main() -> int:
    ...
    ok_a = check_partition()
    print("Test B/C: numerics vs brute force (order 4, eps^2 = 4e-5)")
    ok_b = all(check_numerics(b, z) for b in (4, 5, 6))

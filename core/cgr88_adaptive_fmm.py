...
def p2p_potential_and_force(
    ...
    for i in range(Nt):
        ...
        if exclude_self:
            # Mask out point if exactly same target index in self-box
...
class AdaptiveQuadTree:
    """
    2D Adaptive Quadtree with full CGR88 interaction lists (Lists 1, 2, 3, 4).
    """
    def __init__(
        self,
        ...
        self._build_cgr88_interaction_lists()
    ...
    def _build_cgr88_interaction_lists(self):
        """
        Constructs the 4 CGR88 Interaction Lists:
        ...
        for bid in self.leaves:
            b = self.boxes[bid]
            l1 = []
            # Find all leaves touching b by searching down from root/colleagues
            def find_adjacent_leaves(node_id: int):
                ...
                if node.is_leaf:
                    ...
                    for ch_id in node.children_ids:
                        find_adjacent_leaves(ch_id)
            find_adjacent_leaves(0)  # root
            b.list1 = l1

...
Covers BOTH tables from Farach-Colton, Krapivin, Kuszmaul,
...
def test_elastic_probe_statistics():
    # Measured on the greedy cascade variant: mean ins ~3.8, mean look ~38,
    # max look ~502 at 95% load. Thresholds below are loose caps on those.
    rng = random.Random(2)
    ...
    assert statistics.mean(ins) < 10, statistics.mean(ins)

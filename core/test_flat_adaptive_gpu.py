"""
Emulated and WebGPU cross-validation for the flat adaptive CGR88 kernel.

This validates that the flat layout (adaptive_gpu_metadata.py) and kernel
logic (adaptive_cgr88.wgsl) evaluate the exact same analytical mathematics
as core.cgr88_adaptive_fmm.TreeFreeElasticAdaptiveFMM across orders p=0..4.
"""
...
from .adaptive_gpu_metadata import build_flat_adaptive_metadata, FlatAdaptiveMetadata, INVALID
...
def evaluate_flat_adaptive_emulated(
    ...
    *,
    ...
    Mirrors the full CGR88 adaptive pipeline:
    ...
    for node in range(n_nodes):
        ...
        pts = positions[idxs]
        q = charges[idxs]
        multipoles[node] = cgr88_p2m(pts, q, center, order)
    ...
    for lvl in range(max_depth, -1, -1):
        for node in level_nodes.get(lvl, []):
            ...
            for slot in range(4):
                ...
                acc += cgr88_m2m(multipoles[ch], ch_center, center, order)
            multipoles[node] = acc
    ...
    for lvl in range(1, max_depth + 1):
        for node in level_nodes.get(lvl, []):
            ...
            for src in metadata.list_for(node, 3):
                ...
                pts = positions[idxs]
                q = charges[idxs]
                locals_[node] += cgr88_p2l(pts, q, center, order)
    ...
    for i in range(n_particles):
        ...
        pot = cgr88_l2p(locals_[target], pos_c, tc, order)
        potentials[i] += pot
        lfx, lfy = cgr88_l2p_force(locals_[target], pos_c, tc, order)
        fx[i] += lfx
        fy[i] += lfy
        ...
        for src in metadata.list_for(target, 2):
            ...
            pot_d, deriv_d = cgr88_m2p(multipoles[src], sc, pos_c, order)
            potentials[i] += pot_d
            fx[i] -= deriv_d.real
            fy[i] += deriv_d.imag
